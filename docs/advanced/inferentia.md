...
The versions of `tensorflow-neuron` and `torch-neuron` that are used by Cortex are found in the [Sync API pre-installed packages list](../deployments/syncapi/predictors.md#inferentia-equipped-apis) and [Batch API pre-installed packages list](../deployments/batchapi/predictors.md#inferentia-equipped-apis). When installing these packages with `pip` to compile models of your own, use the extra index URL `--extra-index-url=https://pip.repos.neuron.amazonaws.com`.
...
2. [ResNet50 in PyTorch](https://github.com/cortexlabs/cortex/tree/master/examples/pytorch/image-classifier-resnet50)
