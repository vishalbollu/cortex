...
Here are the complete [Predictor docs](deployments/syncapi/predictors.md).
...
You can skip dependencies that are [pre-installed](deployments/syncapi/predictors.md) to speed up the deployment process. Note that `pickle` is part of the Python standard library so it doesn't need to be included.
...
Here are the complete [API configuration docs](deployments/syncapi/api-configuration.md).
